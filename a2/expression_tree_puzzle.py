...
class ExpressionTreePuzzle(Puzzle):
    ...
    def __str__(self) -> str:
        ...
        for key in self.variables:
            expression += "'" + key + "': " + str(self.variables[key]) + ', '
        expression = expression[:-2]
        expression += '}\n'
        expression += str(self._tree)
        expression += ' = ' + str(self.target)
        return expression
        ...
        for variable in self.variables:
            if self.variables[variable] == 0:
                for i in range(1, 10):
                    ...
                    state = ExpressionTreePuzzle(tree_copy, target)
    ...
    def fail_fast(self) -> bool:
        ...
        if self.extensions() == [] and\
            ...
            return True
        if len(self.extensions()) == 9:
            for state in self.extensions():
                if state._tree.eval(state.variables) == self.target:
                    return False
            return True
        return False
