...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        while r < len(self._grid):
            c = 0
            while c < len(self._grid[r]):
                if self._has_no_allowable_symbols(r, c):
                    return True
                c += 1
            r += 1
        return False
    ...
    def _has_no_allowable_symbols(self, r: int, c: int) -> bool:
        """
        Return True if this cell in row r and column c is empty and has no
        allowable symbols remaining to choose from.
        ...
        ["A", "B", " ", " "], \
        [" ", " ", " ", " "]], {"A", "B", "C", "D"})
        >>> s._has_no_allowable_symbols(2, 3)
        True
        """
    ...
    def has_unique_solution(self) -> bool:
        ...
        if not solver.solve(self, set(str(solution[-1]))):
            return True
        else:
            return False
