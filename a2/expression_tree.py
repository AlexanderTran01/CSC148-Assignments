...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self.is_empty():
            ...
            return self._root
        elif str(self._root) in 'abcdefghijklmnopqrstuvwxyz':
            ...
            total = 1
            for subtree in self._subtrees:
                total = total * subtree.eval(lookup)
            return total
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if str(self._root) in 'abcdefghijklmnopqrstuvwxyz':
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    while not q.is_empty():
        ...
        while i < len(children):
            ...
            parent.append(child)
            if children[i] == OP_ADD or children[i] == OP_MULTIPLY:
